...
# Remove the preload for WaveResource as it's not resolving correctly
# const WaveResource = preload("res://_scripts/wave_resource.gd")

@onready var buddy_mesh = $Buddy/BuddyHead/BuddyMesh
...
func start_next_wave():
	...
	if current_wave_index == 1 and enemy_scene:  # Only one wave
		...
		if wave is Resource:
			var wave_script = wave.get_script()
			if wave_script and wave_script.get_path().get_file() == "WaveResource.gd":
				start_wave(wave)  # Changed from _start_wave to start_wave
			else:
				push_error("Invalid wave resource: Script is not WaveResource")
		else:
			push_error("Invalid wave resource: Not a Resource")
	else:
