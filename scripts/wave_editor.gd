...
const WaveResourceBoy = preload("res://scripts/wave_resource.gd")
...
var wave_resource: WaveResourceBoy
var current_wave_index: int = -1

func _ready():
	wave_resource = WaveResourceBoy.new()
	update_wave_list()
...
func _on_remove_wave_button_pressed():
	if current_wave_index >= 0:
		...
		if wave_resource.waves.size() > 0:
			...
			show_wave_details(WaveResource.new())
